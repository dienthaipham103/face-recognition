...
from tracking_with_object.support_functions import draw_path, show_list, mode, count
...
class TrackingObject:
    ...
    def __str__(self):
        ...
        for i in range(len(self.persons)):
            content += self.persons[i].name + '- '

        # format and display time
        content += '\nTime in: ' + str(self.time_in) + '\nTime out: '
        if self.time_out is not None:
            content += str(self.time_out) + '\nTotal time: ' + str((self.time_out - self.time_in).seconds) + 's'

        # conclusion
        names = [person.name for person in self.persons]
        content += '\nConclusion: ' + self.result + ' with {}/{}'.format(count(names, self.result), len(names))
...
class FaceRecognition:
    ...
    def __init__(self, model_path):
        ...
        # list of tracking objects we removed
        self.removed_tracking_objects = []
    ...
    def delete_face_from_tracking_object(self, face_id):
        # save the time when tracking object is out of frame
        self.tracking_objects[face_id].time_out = datetime.now()

        # get the conclusion who is in tracking object
        names = [person.name for person in self.tracking_objects[face_id].persons]
        result = mode(names)
        self.tracking_objects[face_id].result = result

        # delete and save this tracking object into a list
        self.last_trackers.pop(face_id)
        self.last_persons.pop(face_id)
        self.removed_tracking_objects.append(self.tracking_objects[face_id])
    ...
    def check_deleted_tracker(self, face_id, face_number=5, threshold=10):
        ...
        if face_id in self.out_left_faces.keys():
            ...
            if len(self.out_left_faces[face_id]) == face_number:
                x1 = sum(self.out_left_faces[face_id]) / face_number
                if x1 < 0 + threshold:
                    return face_id
            ...
            # when we have enough 5 positions
            if len(self.out_top_faces[face_id]) == face_number:
                x2 = sum(self.out_top_faces[face_id]) / face_number
                if x2 < 0 + threshold:
                    return face_id
            ...
            # when we have enough 5 positions
            if len(self.out_right_faces[face_id]) == face_number:
                y1 = sum(self.out_right_faces[face_id]) / face_number
                if y1 > 640 - threshold:
                    return face_id
            ...
            # when we have enough 5 positions
            if len(self.out_bottom_faces[face_id]) == face_number:
                y2 = sum(self.out_bottom_faces[face_id]) / face_number
                if y2 > 480 - threshold:
                    return face_id
    ...
    def predict_and_track(self, image, distance_threshold=0.4):
        ...
        # update last_trackers and delete tracking objects
        face_id_to_delete_tracking_object = []
        for face_id in self.last_trackers.keys():
            # update
            self.last_trackers[face_id].update(image)

            # check to delete tracking_objects
            face_id_to_delete = self.check_deleted_tracker(face_id)
            if face_id_to_delete is not None:
                face_id_to_delete_tracking_object.append(face_id_to_delete)

        # delete tracking objects
        for face_id_to_delete in face_id_to_delete_tracking_object:
            self.delete_face_from_tracking_object(face_id_to_delete)

        # get the position of poor trackers that were deleted in the previous frame
        # (not recovered in the previous frame)----------
        for face_id in self.last_trackers.keys():
            print('[INFO] [last trackers] face_id {} => last_tracking position: {}'.
        ...
        for face_id in self.trackers.keys():
            # reset self.out_faces
            if face_id in self.out_left_faces.keys():
                self.out_left_faces.pop(face_id)
            if face_id in self.out_top_faces.keys():
                self.out_top_faces.pop(face_id)
            if face_id in self.out_right_faces.keys():
                self.out_right_faces.pop(face_id)
            if face_id in self.out_bottom_faces.keys():
                self.out_bottom_faces.pop(face_id)

            # get position of tracker before poor tracking filter stage(just in self.trackers-
            # not in self.last_trackers)----------
            print('[INFO] [current trackers] face_id {} => tracking position (before poor tracking filter and update stage): {}'.
            ...
            print('[INFO] [current trackers] face_id {} => tracking quality: {}'.format(face_id, tracking_quality))
        ...
        for face_id in self.trackers.keys():
            print('[INFO] [current tracker] face_id {} => tracking position (after filter poor tracker): {}'
                  .format(face_id, self.trackers[face_id].get_position()))
        ...
        print('[INFO] [faces in frame] face locations: ', face_locations)
        ...
        # not in self.last_trackers) ----------
        for face_id in self.trackers.keys():
            print('[INFO] [current trackers] face_id: {} => tracking position (after recover and find stage- already updated): {}'.
                  format(face_id, self.trackers[face_id].get_position()))
        ...
        if(len(new_face_locations)) > 0:
            ...
            for face_id, name, flag_match, location, distance in zip(new_face_ids, self.knn_clf.predict(faces_encodings),
                ...
                if face_id not in self.tracking_objects.keys():
                    ...
                    tracking_object = TrackingObject(datetime.now())
    ...
    def tracking_object_management(self):
        ...
        for face_id in self.tracking_objects.keys():
            print('[INFO] [tracking objects] face_id: ', face_id)
            print('[INFO] [tracking objects] information of tracking object: ', self.tracking_objects[face_id])
            print('[INFO] [tracking objects] the number of positions: ', len(self.tracking_objects[face_id].positions))

        # check whether the tracking object is empty or not ----------
        if len(self.tracking_objects) == 0:
            print('[INFO] [tracking objects] there is no tracking objects')
...
def draw(frame, persons):
    for name, face_id, location in persons:
        ...
        # print font size ----------
        print('[INFO] [draw] font size: ', font_size)
...
def run_camera():
    ...
    vs = cv2.VideoCapture('D:\\video11.mp4')

    # create object to save video
    fourcc = cv2.VideoWriter_fourcc(*'MJPG')
    out = cv2.VideoWriter('D:\\out.avi', fourcc, 2, (640, 480))
    ...
    while True:
        ...
        print('[INFO] [frame] self.frame: ', recognize.frame_count)
        print('[INFO] [frame] frame: ', real_frame_counter)
        print('[INFO] [current trackers] name, face_id and trackers: ', persons)
        print('[INFO] [general] current_id: ', recognize.current_id)
        print('[INFO] [current trackers] check trackers: ', len(recognize.trackers))
        ...
        # time to end a frame
        end_frame = time.time()
        print('[INFO] [general] process in: {}'.format(end_frame - start_frame))
        print('\n')
        ...
        if len(recognize.tracking_objects) == 0:
            ...
            recognize.trackers = {}
            recognize.persons = {}
            recognize.out_left_faces = {}
            recognize.out_top_faces = {}
            recognize.out_right_faces = {}
            recognize.out_bottom_faces = {}
    ...
    for face_id in recognize.tracking_objects.keys():
        ...
        print('[INFO] [tracking objects] face_id {} => position list (left): '.format(face_id), end='')
        show_list(start_xs)
        print('[INFO] [tracking objects] face_id {} => position list (top): '.format(face_id), end='')
        show_list(start_ys)
        print('[INFO] [tracking objects] face_id {} => position list (right): '.format(face_id), end='')
        show_list(end_xs)
        print('[INFO] [tracking objects] face_id {} => position list (bottom): '.format(face_id), end='')
        show_list(end_ys)
    ...
    # display tracking objects removed
    for tracking_object in recognize.removed_tracking_objects:
        print(tracking_object)

    # check out something after there is no tracking objects
    print(recognize.trackers)
    print(recognize.persons)
    print(recognize.last_trackers)
    print(recognize.last_persons)
    print(recognize.out_left_faces)
    print(recognize.out_top_faces)
    print(recognize.out_right_faces)
    print(recognize.out_bottom_faces)
